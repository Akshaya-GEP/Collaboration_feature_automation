...
import { VariableNodePanel } from '../../pages/orchestration/VariableNodePanel';
...
const workflowName = envFirst('COLLAB_VARIABLE_WORKFLOW_NAME') ?? 'BDD VARIABLE';
const workflowDescription = 'Collaboration test workflow - Variable Node';

const AUTH_A = 'auth.json';
const AUTH_B = 'authB.json';

/**
 * Collaboration test: two browsers, same canvas — Variable node (same flow as guardrail/rule).
 * - Browser A: dashboard, invite B, Create workflow, properties, close panel, Save (no JSON editor).
 * - Browser B: login, open same canvas URL; assert start, output (variable not yet added).
 * - Browser A: click plus (nodes panel), add Variable node, link start → variable → output, open variable node.
 * - Browser B: variable node syncs; open variable node in B. A sets variable update; changes reflect in B.
 */
test.describe('Collaboration - Variable Node, two browsers', () => {
  test('collab variable: A invites B, both open same canvas; A edits variable node and changes reflect in B', async () => {
    ...
    try {
      ...
      test.skip(!name || !description, 'Strict: Name and Description are required. Set COLLAB_VARIABLE_WORKFLOW_NAME in .env if needed.');
      ...
      // ---------- Browser B: assert canvas loaded (start, output — variable not yet added) ----------
      ...
      // ---------- Browser A: add Variable node, link start → variable → output, open variable node ----------
      const graphEditorA = new GraphEditorPage(pageA);
      const nodesPanelA = new NodesPanel(pageA);

      await nodesPanelA.addNodeToCanvasOnly('Variable');
      await pageA.waitForTimeout(1000);

      const startNodeA = graphEditorA.startNode;
      const variableNodeA = graphEditorA.variableNode(0);
      const outputNodeA = graphEditorA.outputNode;
      await graphEditorA.linkNodes(startNodeA, variableNodeA);
      await pageA.waitForTimeout(500);
      await graphEditorA.linkNodes(variableNodeA, outputNodeA);
      await pageA.waitForTimeout(1000);

      await variableNodeA.scrollIntoViewIfNeeded();
      await variableNodeA.dblclick();
      await pageA.waitForTimeout(2000);
      await expect(pageA.getByRole('button', { name: /close panel/i })).toBeVisible({ timeout: 10_000 });

      // ---------- Browser B: variable node should sync; open variable node in B ----------
      await pageB.waitForTimeout(3000);
      await expect(graphEditorB.variableNode(0)).toBeVisible({ timeout: 25_000 });
      await graphEditorB.variableNode(0).scrollIntoViewIfNeeded();
      await graphEditorB.variableNode(0).dblclick();
      await expect(pageB.getByRole('button', { name: /close panel/i })).toBeVisible({ timeout: 15_000 });

      // ---------- Browser A: click Add state, set Select variable and Value; reflect in B (end of flow) ----------
      const variablePanelA = new VariableNodePanel(pageA);
      const variablePanelB = new VariableNodePanel(pageB);

      await variablePanelA.setVariableUpdate({
        fieldName: '{{nodes.start.inputData}}',
        value: '{{system.userQuery}}',
      });
      await pageA.waitForTimeout(1500);
      await variablePanelB.expectVariableUpdateVisible({
        fieldName: '{{nodes.start.inputData}}',
        value: '{{system.userQuery}}',
      });
